program matrixsolve3
  ...
  ! we only store the diagonal elements and superdiagonal values
  ...
  do i = 0,n-1
     diag(i+1) = 1- sum(offdiag(2:n-i))
  enddo

  if (n.eq.10) print *, diag
  ...
  do i = n-1,1,-1
     b(i) = (b(i) - sum(offdiag(2:1+n-i)*b(i+1:n)))/diag(i)
  enddo
