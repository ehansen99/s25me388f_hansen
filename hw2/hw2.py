...
class SourceFree1D():
    ...
    def __init__(self,rmu,lmu,rphi0,lphi0,sigmat,sigmas=0,X=1,Nx=10000):
    ...
    def problem23_forwardflux_banded(self):
        ...
        if self.sigmat < 10**(-10): # Do a naive average in cases of small tau
            self.xrightavg = (self.xright[1:]+self.xleft[:-1])/2
        else:
    ...
    def problem26_plotter(self,problemname):
        ...
        plt.plot(xplot,self.xright,"s",color="mediumslateblue",
                 label="Positive mu",markersize=0.5)
        plt.plot(xplot,self.xleft,"s",color="tomato",
                 label="Negative mu",markersize=0.5)
        plt.plot(xplot,rpsianalytic,"k--",label="Source Free + Solution",markersize=0.5)
        plt.plot(xplot,lpsianalytic,"k--",label="Source Free - Solution",markersize=0.5)
        plt.xlabel("X (cm)")
        ...
        plt.plot(xplot[:-1],self.scalaravg,"s",color="olivedrab",
                 label="Computed",markersize=0.5)
        plt.plot(xplot[:-1],scalaravganalytic,"k:",
                 label="Source Free Solution",markersize=0.5)
