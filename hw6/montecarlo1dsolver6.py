...
class MonteCarlo1DSolver:
    def __init__(self,length,sigmat,sigmas0,q0,Nx,NP,
        ...
        self.sigmas0_material = np.array(sigmas0)
        ...
        self.probscatter = self.sigmas0/(self.sigmat)
        ...
        self.total = self.left*self.psif[0] + np.dot(self.q0,self.length) \
            + self.right*self.psib[1]
        ...
        while mfpdistance_remaining > 0:
            ...
            if self.x >= self.totallength: 
                if self.reflect or self.boundary[1] == 0:
                    ...
                    self.x += 1 # Force particle outside boundary to break particle loop
                    mfpdistance_remaining = 0 # Terminate material loop            
    ...
    def getmoments(self):
        ...
        self.scalarflux = self.scalarflux_distance
    ...
    def plotmoments(self):
        ...
        if len(self.length) == 1:
            plt.title("Scalar Flux "+ str(self.NP) + " Particles "+"Scattering " + ff(self.sigmas0[0],2))
        else:
        ...
        if len(self.length) == 1:
            plt.title("Current "+ str(self.NP) + " Particles "+"Scattering " + ff(self.sigmas0[0],2))
        else:
    ...
    def hw6_sourceposition(self):
        
        r = rand()
        
        # source pdf given by
        # 1.99/2 (e^(-2x)) 0 ≤ x ≤ 35
        # 1.8/2 * e^(-70) * e^(-2(x-35)) 35 ≤ x ≤ 40
        # 2/2 * e^(-80) * e^(-10(x-40)) 40 ≤ x ≤ 50
        
        
        cdf_norm1 = 0.995 * (1/2 - 1/2 * np.exp(-2*35))
        cdf_norm2 = 0.9 * np.exp(-70) * (1/2 - 1/2 * np.exp(-2*5))
        cdf_norm3 = np.exp(-80) * (1/10 - 1/10 * np.exp(-10*10))
        
        cdf_norm = cdf_norm1 + cdf_norm2 + cdf_norm3 
        ...
        if cdf_r < cdf_norm1:
            dr = 0.995/2 - cdf_r
            x = -np.log(2 * dr)/2
        elif cdf_r < cdf_norm1 + cdf_norm2:
            dr = 0.9/2 - (cdf_r - cdf_norm1)/np.exp(-70)
            x = -np.log(2*dr)/2 + 35
        else:
            dr = 1/10 - (cdf_r - cdf_norm1 - cdf_norm2)/np.exp(-80)
            x = -np.log(10*dr)/10 + 40
            
        cdf_norm1 = 1 * (1/2 - 1/2 * np.exp(-2*35))
        cdf_norm2 = 1 * np.exp(-70) * (1/2 - 1/2 * np.exp(-2*5))
        cdf_norm3 = np.exp(-80) * (1/10 - 1/10 * np.exp(-10*10))
        ...
        if cdf_r < cdf_norm1:
            ...
            dr = 1/2 - (cdf_r - cdf_norm1)/np.exp(-70)
            x = -np.log(2*dr)/2 + 35
        else:
            dr = 1/10 - (cdf_r - cdf_norm1 - cdf_norm2)/np.exp(-80)
            x = -np.log(10*dr)/10 + 40
            
        a = np.nonzero(self.cellmesh <= 35)
        self.total = self.dx *  np.sum(0.995 * np.exp(-2*self.cellmesh[a]))
        a = np.nonzero(35 < self.cellmesh * (self.cellmesh <= 40))
        self.total += self.dx * np.sum(0.9 * np.exp(-2*self.cellmesh[a]))
        a = np.nonzero(40 < self.cellmesh)
        self.total += self.dx * np.sum(np.exp(-80)*np.exp(-10*(self.cellmesh[a]-40)))
    ...
    def homework6(self,not1st=False):
        ...
        while self.particleno < self.NP:
            ...
            if not not1st:
                self.x = self.hw6_sourceposition()
            else:
                self.x = self.get_sourceposition()
